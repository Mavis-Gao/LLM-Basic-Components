...
src_vocab = {'P': 0, '我': 1, ': 5,有': 2, '一': 3,
             '个': 4, '好' '朋': 6, '友': 7, '零': 8, '女': 9, '男': 10}
...
embedding_size = 512  # Embedding Size（token embedding和position编码的维度）
...
class PositionalEncoding(nn.Module):
    def __init__(self, embedding_size, dropout=0.1, max_len=5000):
        ...
        # 当前引用绝对位置编码
        pe = torch.zeros(max_len, embedding_size) # 初始化position embedding为size[max_len, embedding_size]的0
        position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1) # unsqueeze(n)代表在前面size[n]的地方增加一个维度，方便后续广播
        div_term = torch.exp(torch.arange(
            0, embedding_size, 2).float() * (-math.log(10000.0) / embedding_size))
        pe[:, 0::2] = torch.sin(position * div_term)
        pe[:, 1::2] = torch.cos(position * div_term)
        pe = pe.unsqueeze(0).transpose(0, 1) # [1, max_len, embedding_size] --> [max_len, 1, embedding_size]
        self.register_buffer('pe', pe) # 将pe变量加载到缓冲区，借用mnodel.save储存到和模型相同的储存设备上，可以通过self.pe直接访问。缓冲区通常用于储存不会被训练的常量数据（pe在模型初始化被计算出来，后续不参与反向传播的参数更新）
    ...
    def forward(self, x):
        ...
        return self.dropout(x) # 对于每一个序列位置、每一个批次都独立处理
...
def get_attn_pad_mask(seq_q, seq_k):
    ...
    pad_attn_mask = seq_k.data.eq(0).unsqueeze(1) # [batch_size, len_k] --> [batch_size, 1, len_k]

    # [batch_size, len_q, len_k] 构成一个立方体(batch_size个这样的矩阵)
    return pad_attn_mask.expand(batch_size, len_q, len_k)


# 创建序列掩码，即Subsequence Mask，用于Decoder的掩码子注意力Masked Self-Attention解码时只能看到当前位置及之前的信息
def get_attn_subsequence_mask(seq):
    ...
    subsequence_mask = np.triu(np.ones(attn_shape), k=1)  # 生成全1矩阵-->k=1表示主对角线向上为1（不包括主对角线）
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        scores.masked_fill_(attn_mask, -1e9) # 将掩码为True的位置填充为极小值，令其在softmax中权重接近0

        attn = nn.Softmax(dim=-1)(scores)  # 对最后一个维度(v)做softmax，输出size[batch_size, n_heads, len_q, len_k]
...
class MultiHeadAttention(nn.Module):
    ...
    输入：seq_len x embedding_size
    输出：seq_len x embedding_size
    """
    ...
    def __init__(self):
        ...
        self.W_V = nn.Linear(embedding_size, d_v * n_heads, bias=False)
        # 这个全连接层可以保证多头attention的输出仍然是seq_len x embedding_size
        self.fc = nn.Linear(n_heads * d_v, embedding_size, bias=False)

    def forward(self, input_Q, input_K, input_V, attn_mask):
        """
        input_Q: [batch_size, len_q, embedding_size]
        input_K: [batch_size, len_k, embedding_size]
        input_V: [batch_size, len_v(=len_k), embedding_size]
        ...
        # 这个全连接层可以保证多头attention的输出仍然是seq_len x embedding_size
        output = self.fc(context)  # [batch_size, len_q, embedding_size]
        return nn.LayerNorm(embedding_size).to(device)(output + residual), attn
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_inputs, enc_self_attn_mask):
        """E
        enc_inputs: [batch_size, src_len, embedding_size]
        enc_self_attn_mask: [batch_size, src_len, src_len]  mask矩阵(pad mask or sequence mask)
        """
        # enc_outputs: [batch_size, src_len, embedding_size], attn: [batch_size, n_heads, src_len, src_len]
        # 第一个enc_inputs * W_Q = Q
        # 第二个enc_inputs * W_K = K
        # 第三个enc_inputs * W_V = V
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_inputs, enc_outputs, dec_self_attn_mask, dec_enc_attn_mask):
        """
        dec_inputs: [batch_size, tgt_len, embedding_size]
        enc_outputs: [batch_size, src_len, embedding_size]
        dec_self_attn_mask: [batch_size, tgt_len, tgt_len]
        dec_enc_attn_mask: [batch_size, tgt_len, src_len]
        """
        # dec_outputs: [batch_size, tgt_len, embedding_size], dec_self_attn: [batch_size, n_heads, tgt_len, tgt_len]
        dec_outputs, dec_self_attn = self.dec_self_attn(dec_inputs, dec_inputs, dec_inputs,
                                                        dec_self_attn_mask)  # 这里的Q,K,V全是Decoder自己的输入
        # dec_outputs: [batch_size, tgt_len, embedding_size], dec_enc_attn: [batch_size, h_heads, tgt_len, src_len]
        dec_outputs, dec_enc_attn = self.dec_enc_attn(dec_outputs, enc_outputs, enc_outputs,
                                                      dec_enc_attn_mask)  # Attention层的Q(来自decoder) 和 K,V(来自encoder)
        # [batch_size, tgt_len, embedding_size]
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.src_emb = nn.Embedding(src_vocab_size, embedding_size)  # token Embedding
        self.pos_emb = PositionalEncoding(
            embedding_size)  # Transformer中位置编码时固定的，不需要学习
    ...
    def forward(self, enc_inputs):
        ...
        # pos_emb对应class PositionEncoding的输入size为[seq_len, batch_size, embedding_size]
        enc_outputs = self.pos_emb(enc_outputs.transpose(0, 1)).transpose(
            0, 1)  # -->[batch_size, src_len, embedding_size]
        ...
        for layer in self.layers:  # for循环访问nn.ModuleList对象
            ...
            # enc_outputs: [batch_size, src_len, embedding_size], enc_self_attn: [batch_size, n_heads, src_len, src_len]
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.tgt_emb = nn.Embedding(
            tgt_vocab_size, embedding_size)  # Decoder输入的embed词表
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):
        ...
        enc_inputs: [batch_size, src_len]
        enc_outputs: [batch_size, src_len, embedding_size]   # 用在Encoder-Decoder Attention层
        """
        dec_outputs = self.tgt_emb(
            dec_inputs)  # [batch_size, tgt_len, embedding_size]
        dec_outputs = self.pos_emb(dec_outputs.transpose(0, 1)).transpose(0, 1).to(
            device)  # [batch_size, tgt_len, embedding_size]
        ...
        for layer in self.layers:
            # dec_outputs: [batch_size, tgt_len, embedding_size], dec_self_attn: [batch_size, n_heads, tgt_len, tgt_len], dec_enc_attn: [batch_size, h_heads, tgt_len, src_len]
            # Decoder的Block是上一个Block的输出dec_outputs（变化）和Encoder网络的输出enc_outputs（固定）
            dec_outputs, dec_self_attn, dec_enc_attn = layer(dec_outputs, enc_outputs, dec_self_attn_mask,
                                                             dec_enc_attn_mask)
            dec_self_attns.append(dec_self_attn)
            dec_enc_attns.append(dec_enc_attn)
        # dec_outputs: [batch_size, tgt_len, embedding_size]
...
class Transformer(nn.Module):
    def __init__(self):
        ...
        self.projection = nn.Linear(
            embedding_size, tgt_vocab_size, bias=False).to(device)
    ...
    def forward(self, enc_inputs, dec_inputs):
        ...
        # enc_outputs: [batch_size, src_len, embedding_size], enc_self_attns: [n_layers, batch_size, n_heads, src_len, src_len]
        # 经过Encoder网络后，得到的输出还是[batch_size, src_len, embedding_size]
        enc_outputs, enc_self_attns = self.encoder(enc_inputs)
        # dec_outputs: [batch_size, tgt_len, embedding_size], dec_self_attns: [n_layers, batch_size, n_heads, tgt_len, tgt_len], dec_enc_attn: [n_layers, batch_size, tgt_len, src_len]
        dec_outputs, dec_self_attns, dec_enc_attns = self.decoder(
            dec_inputs, enc_inputs, enc_outputs)
        # dec_outputs: [batch_size, tgt_len, embedding_size] -> dec_logits: [batch_size, tgt_len, tgt_vocab_size]
